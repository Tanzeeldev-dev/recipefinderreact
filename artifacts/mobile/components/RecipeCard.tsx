...
export function RecipeCard({ recipe, variant = "compact" }: RecipeCardProps) {
  ...
  const handleFavorite = () => toggleFavorite(recipe.id);

  const imageSource = imgError
    ? { uri: `https://picsum.photos/seed/${recipe.id}/600/400` }
    : { uri: recipe.image };
  ...
  if (variant === "featured") {
    return (
      ...
      >
        <View style={[styles.featuredImageWrap, { backgroundColor: colors.muted }]}>
          <Image
            source={imageSource}
  ...
  if (variant === "list") {
    return (
      ...
      >
        <View style={[styles.listImageWrap, { backgroundColor: colors.muted }]}>
          <Image
            source={imageSource}
        ...
        <View style={styles.listInfo}>
          <Text style={[styles.listCategory, { color: colors.primary }]} numberOfLines={1}>
            {recipe.category}
          </Text>
          ...
          <View style={styles.listMeta}>
            <View style={styles.metaItem}>
              ...
              <Text style={[styles.listMetaText, { color: colors.mutedForeground }]}>{recipe.time}m</Text>
            </View>
            <View style={styles.metaItem}>
              <Ionicons name="star" size={13} color="#FFB300" />
              <Text style={[styles.listMetaText, { color: colors.mutedForeground }]}>{recipe.rating}</Text>
            </View>
            <Text
              style={[
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.compactCard,
        { backgroundColor: colors.card, width: COMPACT_CARD_WIDTH, opacity: pressed ? 0.9 : 1 },
      ]}
    >
      <View style={[styles.compactImageWrap, { backgroundColor: colors.muted }]}>
        <Image
          source={imageSource}
      ...
      <View style={styles.compactInfo}>
        ...
        <View style={styles.compactMeta}>
          ...
          <Text style={[styles.compactTime, { color: colors.mutedForeground }]}>{recipe.time}m</Text>
          <Ionicons name="star" size={11} color="#FFB300" />
          <Text style={[styles.compactTime, { color: colors.mutedForeground }]}>{recipe.rating}</Text>
        </View>
      </View>
    </Pressable>
  );
...
const styles = StyleSheet.create({
  featuredCard: { width: CARD_WIDTH, height: 220, borderRadius: 20, overflow: "hidden", marginLeft: 8 },
